...
class Core():
    def __init__(self, imem, sdmem, vdmem):
        ...
        for i in range(64):#vector mask register is cleared that is all bits are 1 by default.
            vmr.registers[0][i]=0
        l1=[]
        for i in range(450):
            l1.append(int(vdmem.data[i]))
        i=450
        l2=[]
        while(i<900):
            l2.append(int(vdmem.data[i]))
            i=i+1
        l4=0
        for i in range(450):
            l4=l4+(l1[i]*l2[i])
